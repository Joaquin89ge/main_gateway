 ...
 */
#define RFM95_CS 4 // d3

/**
 * @brief Pin de Reinicio (RST) para el módulo LoRa RFM95.
 *
 * Este pin se utiliza para realizar un reinicio por hardware del módulo RFM95.
 * Corresponde al pin  digital D2 en algunas placas de desarrollo.
 */
#define RFM95_RST 16 // d2
...
{
    // Configura el pin de reinicio del RFM95 como salida.
    pinMode(RFM95_RST, OUTPUT);
    // Realiza una secuencia de reinicio para el módulo RFM95.
    ...
    // Intenta inicializar el gestor RHMesh.
    if (!manager.init())
    {
        Serial.println("Fallo al inicializar RHMesh");
        return false;
    }

    // Establece la frecuencia de operación de LoRa.
    driver.setFrequency(FRECUENCI_CONF); // Cambiar si es necesario
    // Establece la potencia de transmisión de LoRa al máximo (20 dBm).
    driver.setTxPower(POWER_LORA, false); // Máxima potencia

    Serial.println("RadioManager iniciado con éxito");
    return true;
}
